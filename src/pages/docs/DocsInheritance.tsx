...
const STEPS = [
  ...
  },
  {
    ...
    items: [
      ...
      "(Desktop App) Every Desktop-exported Qard has a truncated SHA-256 fingerprint printed on its face. Record each Qard's fingerprint in your letter of instruction or inheritance ledger — decades later, heirs can compare the printed hash against the recorded hash to detect Qard substitution without scanning anything or trusting any device. The defense relies on the external record: hash alone is not tamper-proof (a forger could compute and print a self-consistent hash on a substituted card), but hash-paired-with-recorded-reference is.",
